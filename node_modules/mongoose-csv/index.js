    ...
    schema.methods.toCSV = function() {
        ...
        for (var property in doc._doc) {
            if (doc._doc[property] instanceof Date) {
                doc._doc[property] = moment(doc._doc[property]).utc().format('YYYY-MM-DD');
            }
        }
...
function prop_to_csv(prop) {
    var val = String(prop);
    if (prop && prop.toCSVString) {
        val = prop.toCSVString;
    }
    if (!val || val === 'undefined') val = '';
    // console.log(JSON.stringify(val) + " -> " + JSON.stringify(prop));
